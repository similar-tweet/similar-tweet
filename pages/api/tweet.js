...
function extractTweets(data){
   let tweets = Object.values(data.globalObjects.tweets).filter(x => x.in_reply_to_status_id == null);
...
function getMoreTweets(tweetId){
    ...
    fetch(url, {
        method: 'GET',
        headers: headers
    }).then(resp => resp.json()).then(data => extractTweets(data)).catch(e => console.log(e));
}
